...
Route::prefix('v1')->group(function (): void {
    // Unauthenticated health check — no auth:sanctum required.
    Route::match(['get', 'head'], 'health', fn () => response()->json(['ok' => true], 200));
    ...
    Route::middleware(['auth:sanctum', 'active_shop', 'shop_scope'])->group(function (): void {
        ...
        Route::match(['put', 'patch'], 'products/{product}', [ProductController::class, 'update'])->middleware(['api_ability:products,update', 'idempotent']);
        Route::delete('products/{product}', [ProductController::class, 'destroy'])->middleware(['api_ability:products,delete', 'idempotent']);
        ...
        Route::delete('expenses/{expense}', [ExpenseController::class, 'destroy'])->middleware(['api_ability:expenses,delete', 'idempotent']);
        ...
        Route::match(['put', 'patch'], 'debts/{debt}', [DebtController::class, 'update'])->middleware(['api_ability:debts,update', 'idempotent']);
        ...
        Route::match(['put', 'patch'], 'purchases/{purchase}', [PurchaseController::class, 'update'])->middleware(['api_ability:purchases,update', 'idempotent']);
        Route::delete('purchases/{purchase}', [PurchaseController::class, 'destroy'])->middleware(['api_ability:purchases,delete', 'idempotent']);
        ...
        Route::post('notifications/token', [NotificationController::class, 'store'])->middleware(['idempotent', 'api_ability:notifications,create', 'throttle:mobile-writes']);
    });
});
